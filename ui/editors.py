...
class _BrushEditor(Gtk.VBox):
    ...
    def on_blur_scale_value_changed(self, source):
        ...
        # self.canvas.imag.img[self.canvas.current_selection] = np.ravel(cv2.blur(self.canvas.imag.img,
        #                                                                         (scale_value, scale_value)))
        self.canvas.imag.img[self.canvas.current_selection] = np.ravel(np.array(PIL.Image.fromarray(
            self.canvas.imag.img).filter(ImageFilter.GaussianBlur(scale_value))))
        self.canvas.refresh()
